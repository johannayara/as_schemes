...
impl Client {
    pub fn new(scheme:Scheme) -> Self {
        ...
        Self { sk_c, pk_c, scheme }
    }

    pub fn generate_presig(&self, ct: &[u8], server_pk: &ProjectivePoint) -> Delta_prime {
        let r_prime = Scalar::random(&mut OsRng);
        let delta_prime = self.scheme.pre_sign(&self.sk_c, &hex::encode(ct), server_pk, &r_prime);
        delta_prime
    }
    
    pub fn verify_sign(&self, server_pk: &ProjectivePoint, ct: &[u8], delta_s: &Delta, delta_c: &Delta) -> bool{
        let is_s_correct = self.scheme.verify_sign(delta_s,server_pk, &hex::encode(ct));
        let is_c_correct = self.scheme.verify_sign(delta_c, &self.pk_c, &hex::encode(ct));
